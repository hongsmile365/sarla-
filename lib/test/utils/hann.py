...
def hann1d(sz: int, centered=True, offset=0.0) -> torch.Tensor:
    """1D cosine window with an offset."""
    if centered:
        return 0.5 * (1 - torch.cos((2 * math.pi / (sz + 1)) * (torch.arange(1, sz + 1).float() + offset)))
    w = 0.5 * (1 + torch.cos((2 * math.pi / (sz + 2)) * (torch.arange(0, sz//2 + 1).float() + offset)))
    return torch.cat([w, w[1:sz-sz//2].flip((0,))])

def hann2d(sz: torch.Tensor, centered=True, offset: torch.Tensor = torch.tensor([0.0, 0.0])) -> torch.Tensor:
    """2D cosine window with an offset."""
    return hann1d(sz[0].item(), centered, offset[0].item()).reshape(1, 1, -1, 1) * hann1d(sz[1].item(), centered, offset[1].item()).reshape(1, 1, 1, -1)
...
def max2d(a: torch.Tensor) -> (torch.Tensor, torch.Tensor):
    """Computes maximum and argmax in the last-bak two dimensions."""
