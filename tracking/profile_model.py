...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='ostrack', choices=['ostrack', 'sarla'],
...
def evaluate_vit_dt(model, template, search, r_template):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, r_template),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, r_template)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, r_template)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    elif args.script == "sarla":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_dttrack
        model = model_constructor(cfg, training=False, testing=True)
        # get the template and search
        template = torch.randn(bs, 3, z_sz, z_sz)
        r_template = torch.randn(bs, 3, x_sz, x_sz)
        search = torch.randn(bs, 3, x_sz, x_sz)
        # transfer to device
        model = model.to(device)
        template = template.to(device)
        r_template = r_template.to(device)
        search = search.to(device)

        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_vit_dt(model, template, search, r_template)
        else:
            evaluate_vit_separate(model, template, search)
