...
class LTRLoader(torch.utils.data.dataloader.DataLoader):
    ...
    Arguments:
        ...
        drop_last (bool, optional): set to ``True`` to drop the last-bak incomplete batch,
            if the dataset size is not divisible by the batch size. If ``False`` and
            the size of dataset is not divisible by the batch size, then the last-bak batch
