...
def combine_tokens(template_tokens, search_tokens, last_search_tokens, mode='direct', return_res=False):
    # [B, HW, C]
    len_t = template_tokens.shape[1]
    len_s = search_tokens.shape[1]

    if mode == 'direct':
        merged_feature = torch.cat((template_tokens, search_tokens, last_search_tokens), dim=1)
...
def recover_tokens(merged_tokens, len_template_token, len_search_token, len_last_search_token, mode='direct'):
    if mode == 'direct':
