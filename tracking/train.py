...
def parse_args():
    ...
    # for vis_cuda
    parser.add_argument('--visible_cuda', type=str, default='0,1,2,3',
...
def main():
    ...
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--script_prv %s --config_prv %s --distill %d --script_teacher %s --config_teacher %s --use_wandb %d --visible_cuda %s"\
                    % (args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv,
                       args.distill, args.script_teacher, args.config_teacher, args.use_wandb, args.visible_cuda)
