...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, last_frame_gap=1):
        """
        args:
        ...
        # 跨模态数据
        self.cross_dict, self.cross_number = datasets[0].get_cross_modal()
        self.last_frame_gap = last_frame_gap
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
                            allow_invisible=False, force_invisible=False, last_number=0):
        ...
        if force_invisible:
            ...
            if allow_invisible:
                ...
                # min_id + 1 是为了保证两个search都在min_id 到 max_id之间
                valid_ids = [i for i in range(min_id + last_number, max_id) if visible[i] and visible[i - last_number]]
    ...
    def __getitem__(self, index):
        ...
        if self.train_cls:
            ...
            if index < self.cross_number:
                return self.get_cross_item(index)
            # elif self.cross_number <= index < self.cross_number*2:
            #     return self.get_reversed_cross_item(index)
            else:
                return self.getitem()
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            last_number = random.randint(1, self.last_frame_gap)
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        base_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_template_frames - 1,
                                                                 max_id=len(visible) - self.num_search_frames, last_number=last_number)
                        prev_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,
                                                                  min_id=base_frame_id[0] - self.max_gap - gap_increase,
                                                                  max_id=base_frame_id[0], last_number=last_number)
                        ...
                        search_frame_ids = self._sample_visible_ids(visible, min_id=template_frame_ids[0] + 1,
                                                                  ...
                                                                  num_ids=self.num_search_frames, last_number=last_number)
            ...
            try:
                ...
                last_search_frame_ids = [id - last_number for id in search_frame_ids]
                # TODO: handle the case when the last-bak search frame is not visible
                last_search_frames, last_search_anno, meta_obj_test = dataset.get_frames(seq_id, last_search_frame_ids, seq_info_dict)
                # 可视化
                # cv2.imwrite('1.jpg', template_frames[0])
                ...
                last_search_masks = last_search_anno['mask'] if 'mask' in last_search_anno else [torch.zeros((H, W))] * self.num_search_frames

                data = TensorDict({'template_images': template_frames,
                                   'template_anno': template_anno['bbox'],
                                   'template_modal': template_anno['modal'],
                                   'template_masks': template_masks,
                                   'search_images': search_frames,
                                   'search_anno': search_anno['bbox'],
                                   'search_modal': search_anno['modal'],
                                   'search_masks': search_masks,
                                   'last_search_images': last_search_frames,
                                   'last_search_anno': last_search_anno['bbox'],
                                   'last_search_modal': last_search_anno['modal'],
                                   'last_search_masks': last_search_masks,
    ...
    def get_cross_item(self, index):
        """
        returns:
            TensorDict - dict containing all the data blocks
        """
        index = index % self.cross_number
        valid = False
        while not valid:
            seq_id, search_id = map(int, self.cross_dict[index].split("@"))
            ...
            if self.frame_sample_mode == 'causal':
                ...
                template_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames,
                                                          min_id=search_id - self.max_gap,
                                                          max_id=search_id)

                search_frame_ids.append(search_id)
            ...
            try:
                ...
                search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids,
                                                                               seq_info_dict)
                last_search_frame_ids = [id - 1 for id in search_frame_ids]
                # TODO: handle the case when the last-bak search frame is not visible
                last_search_frames, last_search_anno, meta_obj_test = dataset.get_frames(seq_id,
                                                                                             last_search_frame_ids,
                                                                                             seq_info_dict)
                ...
                search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros(
                    (H, W))] * self.num_search_frames
                last_search_masks = last_search_anno['mask'] if 'mask' in last_search_anno else [torch.zeros(
                    (H, W))] * self.num_search_frames
    ...
    def get_reversed_cross_item(self, index):
        """
        returns:
            TensorDict - dict containing all the data blocks
        """
        ...
        while not valid:
            ...
            if self.frame_sample_mode == 'causal':
                ...
                template_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames,
                                                          min_id=search_id,
                                                          max_id=search_id + self.max_gap)

                search_frame_ids.append(search_id - 1)
