...
class OffsetPredictor(nn.Module):
    def __init__(self, inplanes=64, channel=256*2, feat_sz=20, stride=16, freeze_bn=False):
    ...
    def forward(self, x):
        x_offset1 = self.conv1_offset(x)
        x_offset1 = self.pool1(x_offset1)
        x_offset2 = self.conv2_offset(x_offset1)
        x_offset2 = self.pool2(x_offset2)
        x_offset3 = self.conv3_offset(x_offset2)
        x_offset3 = self.pool3(x_offset3)
        x_offset4 = self.conv4_offset(x_offset3)
        x_offset4 = self.pool4(x_offset4)
        score_map_offset = self.conv5_offset(x_offset4)
        return score_map_offset
...
def build_align_head(cfg, hidden_dim):
    stride = cfg.MODEL.BACKBONE.STRIDE

    in_channel = hidden_dim
    out_channel = cfg.MODEL.HEAD.NUM_CHANNELS*2
    feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)

    align_head = OffsetPredictor(inplanes=in_channel, channel=out_channel,
                                 feat_sz=feat_sz, stride=stride)
