...
# TODO: Implement a function to read a .txt file and return a tensor
def read_txt_file(file_path):
    """
        Reads a .txt file and returns a tensor.

        Args:
        file_path (str): Path to the .txt file.

        Returns:
        tensor: Tensor containing the contents of the .txt file.
        """
    with open(file_path, 'r') as file:
        data = file.readlines()
    tensor = torch.tensor([float(item) for item in data])
    return tensor
def init_seeds(seed):
    random.seed(seed)
    np.random.seed(seed)
...
def main():
    ...
    # for vis_cuda
    parser.add_argument('--visible_cuda', type=str, default='3',
                        help='Comma-separated list of CUDA device ids that are visible.')
