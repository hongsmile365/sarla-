...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['last_search_images'], data['last_search_anno'], data['last_search_masks'] = self.transform['joint'](
                image=data['last_search_images'], bbox=data['last_search_anno'], mask=data['last_search_masks'], new_roll=False)

        for s in ['template', 'last_search', 'search']:
            assert self.mode == 'sequence' or len(data[s + '_images']) == 1, \
                "In pair mode, num train/test frames must be 1"

            # Add a uniform noise to the center pos
            # 相邻两帧搜索区域保持同一个crop center and scale

            if s != 'search':
                jittered_anno = [self._get_jittered_box(a, s.split('_')[-1]) for a in data[s + '_anno']]

            # 2021.1.9 Check whether data is valid. Avoid too small bounding boxes
            w, h = torch.stack(jittered_anno, dim=0)[:, 2], torch.stack(jittered_anno, dim=0)[:, 3]

            crop_sz = torch.ceil(torch.sqrt(w * h) * self.search_area_factor[s.split('_')[-1]])
            if (crop_sz < 1).any():
                data['valid'] = False
                # print("Too small box is found. Replace it with new data.")
                return data

            # frame = data[s + '_images'][0]
            # anno = jittered_anno[0].tolist()
            # cv2.rectangle(frame, (int(anno[0]), int(anno[1])),
            #               (int(anno[0] + anno[2]), int(anno[1] + anno[3])), (0, 255, 0), 2)
            # cv2.rectangle(frame, (int(anno[0]), int(anno[1])),
            #               (int(anno[0] + crop_sz), int(anno[1] + crop_sz)), (255, 0, 0), 2)
            ...
            crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data[s + '_images'], jittered_anno,
                                                                              data[s + '_anno'], self.search_area_factor[s.split('_')[-1]],
                                                                              self.output_sz[s.split('_')[-1]], masks=data[s + '_masks'])
            # res = crops[0]
            # sz = res.shape[0]
            # mybox = boxes[0].tolist()
            # cv2.rectangle(res, (int(mybox[0]*sz), int(mybox[1]*sz)),
            #               (int(mybox[0]*sz + mybox[2]*sz), int(mybox[1]*sz + mybox[3]*sz)), (0, 255, 0), 2)
            # cv2.imwrite(s + '.jpg', res)
            # Apply transforms
            data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s.split('_')[-1]](
            ...
            for ele in data[s + '_att']:
                feat_size = self.output_sz[s.split('_')[-1]] // 16  # 16 is the backbone stride
        ...
        if data["template_masks"] is None or data["search_masks"] is None:
            ...
            data["last_search_masks"] = torch.zeros((1, self.output_sz["search"], self.output_sz["search"]))
        # Prepare output
